...
infile_Sean_MESH = '/glade/work/guoqiang/CTSM_cases/CAMELS_Calib/shared_data_Sean/ESMFmesh_ctsm_HCDN_nhru_final_671.buff_fix_holes_polygons_simplified_5e-4_no_nested.nc'
...
shp_camels = shp_camels.drop(columns=['geometry'])
shp_lat_cen = shp_camels['lat_cen'].values
shp_lon_cen = shp_camels['lon_cen'].values
...
for i in range(ds_mesh.elementCount.size):
    ...
    shpi = shp_camels[indexi:indexi+1]
    shpi = shpi.reindex()
