...
def calib_period_Ending(data, date, calibyears, validratio, wy_month):
    # find the period from the ending of the series (e.g., the most recent 6 years are used for calibration)

    # df_q: dataframe of Q. It must contain two columns: date and Qobs
    # calibyears: int, number of calibration years
    # validratio: the ratio of valid sample numbers during the selected period
    # wy_month: e.g., the water year in CONUS starts in October, so the calibration period should be Sep to Oct

    print('Decide calibration period using the ending choice')

    # decide the trial start date
    trial_end_date = date[-1]
    year = trial_end_date.astype('datetime64[Y]')
    trial_end_date = pd.Timestamp(f'{year}-{wy_month}-01') - pd.offsets.DateOffset(hours=1) 

    # find a calibration period that meets validratio
    flag = True
    while flag:
        trial_start_date = trial_end_date - pd.offsets.DateOffset(years=calibyears) + pd.offsets.DateOffset(hours=1) 
        index = (date >= trial_start_date) & (date <= trial_end_date)
        if np.sum(index) > calibyears*365*validratio:
            qtrial = data[index]
            if np.sum(qtrial >= 0) / len(qtrial) >= validratio:
                flag = False
            else:
                trial_end_date = trial_end_date - pd.offsets.DateOffset(years=1)
        else:
            flag = False
            print('Stop searching because reaching the beginning of the series ...')
...
if __name__ == '__main__':
    ...
    trial_start_date, trial_end_date = calib_period_Ending(data, date, calibyears=5, validratio=0.8, wy_month=10)
    # method-2:
    print('#'*50)
    print('Method-2')
    df_q = read_raw_CAMELS_Q_to_df(infile_q)
