...
outpath_case = '/glade/work/guoqiang/CTSM_cases/CAMELS_Calib/Calib_all_HH_MOASMO'
outpath_out = '/glade/scratch/guoqiang/CTSM_outputs/CAMELS_Calib/Calib_all_HH_MOASMO'
...
config_calib['files']['path_calib'] = f"/glade/campaign/cgd/tss/people/guoqiang/CTSMcases/CAMELS_Calib/Calib_all_HH_MOASMO/{level}_{basin_num}_MOASMOcalib" # if not provided, just use default settings (i.e., a folder within the same folder with the CTSM case)
...
config_calib['settings']['num_init'] = 180 # initial number of samples
...
config_calib['job']['job_controlMOASMO'] = ['#PBS -N MOAcontrol', '#PBS -q regular', '#PBS -l select=1:ncpus=36', '#PBS -l walltime=12:00:00', f'#PBS -A {projectcode}']
...
if config_calib['job']['job_mode'] == 'lumpsubmit_allinone':
    ...
    config_calib['job']['job_CTSMiteration'] = ['#PBS -N CTSMiter', '#PBS -q regular', '#PBS -l select=1:ncpus=36', '#PBS -l walltime=12:00:00', f'#PBS -A {projectcode}']
