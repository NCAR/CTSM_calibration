...
def run_trial(params):
    basin, trialflag, configfile = params
    config = toml.load(configfile)
    ...
    caseflag = f'iter0_trial{trialflag}'
    ...
    if not os.path.isfile(outfile_metric):
        ...
        if len(infilelist) > 0:
            ...
            print(f"No input files found for trial {trialflag} in basin {basin}.")
...
if __name__ == '__main__':
    ...
    for basin in range(basin_num):
        ...
        for trialflag in range(400):
            if np.mod(trialflag, 100) == 0:
                print(trialflag)
            tasks.append((basin, trialflag, configfile))
