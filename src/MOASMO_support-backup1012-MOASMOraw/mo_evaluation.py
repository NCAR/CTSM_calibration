...
def mo_evaluate_return_many_metrics(outfile_metric, CTSMfilelist, fsurdat, date_start, date_end, ref_streamflow, shift=0, add_flow_file=''):
...
def mo_evaluate_return_many_metrics_mosart(outfile_metric, CTSMfilelist, date_start, date_end, ref_streamflow, shift=0, latind=None, lonind=None):
    ...
    clm_q_name = 'RIVER_DISCHARGE_OVER_LAND_LIQ'
    ref_q_name = 'Runoff_cms'
    ref_q_date = 'Date'
    ...
    try:
        ...
        if latind!=None and lonind!=None:
            if 'time' in ds_simu.mask.dims:
                m = ds_simu.mask.isel(time=0, lat=latind, lon=lonind).values
            else:
                m = ds_simu.mask.isel(lat=latind, lon=lonind).values
            
            if m != 1:
                print('Target grid has mask != 1')
                print('Use another method instead latind/lonind:', latind, lonind)
                latind, lonind = None, None
        
        
        if latind==None or lonind==None:
            # select one grid with the largest mean flow
            # only used for subset MOSART domain
            mean_values = ds_simu[clm_q_name].mean(dim='time')
            m1, m2 = np.unravel_index(np.nanargmax(mean_values.values), mean_values.shape)  # Get the indices
            ds_simu = ds_simu.isel(lat=m1, lon=m2)
        else:
            ds_simu = ds_simu.isel(lat=latind, lon=lonind)
    ...
    if np.all(ds_simu[clm_q_name].values == 0):
        print('All 0 flow data. No need to evaluate')
        return
...
def mo_evaluate_mosart(outfile_metric, CTSMfilelist, date_start, date_end, ref_streamflow):
...
def mo_evaluate_return_many_metrics_mizuroute(outfile_metric, mizufilelist, date_start, date_end, ref_streamflow, outlet_segId=None, shift=0,):
    ...
    # load MOSART m3/s
    try:
        ds_simu = xr.open_mfdataset(mizufilelist)
        ...
        print('Fail to read CTSMfilelist:', mizufilelist)
        return

    
    if len(ds_simu) == 0:
        print('Not enough files in CTSMfilelist:', mizufilelist)
        return
