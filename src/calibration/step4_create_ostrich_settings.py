...
# sys input arguments
inpathCTSMcase = sys.argv[1]
inpathOstSource = sys.argv[2]
infileCalibParam = sys.argv[3]
infile_Qobs = sys.argv[4]
DateEvalStart = sys.argv[5]
DateEvalEnd = sys.argv[6]
...
# infile_Qobs = '/glade/p/ral/hap/common_data/camels/obs_flow_met/basin_dataset_public_v1p2/usgs_streamflow/all/01022500_streamflow_qc.txt'
# DateEvalStart = '2000-01-01'
# DateEvalEnd = '2002-12-31'

#############
# default settings
projectcode = 'P08010000'

outpathOstCalib = inpathCTSMcase + "_OstCalib"
...
# template files
infile_ostin_template = f'{inpathOstSource}/ostIn_KGE_DDS.tpl'
infile_runtrial_template = f'{inpathOstSource}/CTSM_run_trial.sh'
infile_savebest_template = f'{inpathOstSource}/call_PreserveBestModel.sh'
infile_savemodel_template = f'{inpathOstSource}/call_PreserveModelOutput.sh'

# scripts
script_gen_ostIn = f'{inpathOstSource}/auxiliary_create_ostin.py'
script_save_file = f'{inpathOstSource}/save_Ostrich_trial_outputs.py'

infile_lndin = f'{inpathCTSMcase}/Buildconf/clmconf/lnd_in'
...
df_calibparam = pd.read_csv(infileCalibParam)
...
_ = subprocess.run(f'python {script_gen_ostIn} {infile_lndin} {infile_ostin_template} {infileCalibParam} {outfile_ostin_txt}', shell=True)
...
runtrial_setting = {}
runtrial_setting['pathCTSMcase'] = inpathCTSMcase
...
save_setting['pathCTSM'] = inpathCTSMcase
...
lines2 = []
template_file = f'{inpathCTSMcase}/.case.run'
...
df_q_in = pd.read_csv(infile_Qobs, delim_whitespace=True, header=None)
